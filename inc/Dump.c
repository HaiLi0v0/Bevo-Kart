...
// The software simply reads TIMG12->COUNTERREGS.CTR to get the current time in bus cycles.
  // check if hit the max 
  // if not then add to the array and log the time
  uint32_t i = DebugCnt;
  ...
  if( DebugCnt >= 50){
    ...
    DataBuffer[i] = data;
    TimeBuffer[i] = TIMG12->COUNTERREGS.CTR;
    DebugCnt += 1;
    return 1; // success
  }
}
...
//    if zero, the signal is considered low.
 // if debugcnt is > 2, do period calculations:
    //find the rising edge and stop the loop when it finds two rising edges 
    //uint32_t size = MAXBUF;
    uint32_t t1=0;
    uint32_t t2=0;
    
    // used to check if the first edge was found
    // when 0 then found first edge
    uint8_t cnt=1;
    
    if (DebugCnt > 2){ // checking if there is enough data to be used
      
      for (uint32_t i = 1; i < DebugCnt; i++ ){
        // prev and current, checks low to high
        uint32_t prev = (DataBuffer[i-1] & mask) != 0 ;
        uint32_t cur = (DataBuffer[i] & mask) != 0;
        
        // if prev was 0 and cur is 1 then continue
        if(!prev && cur){ 
          // check if the first time was found
          if(cnt){
            // time found, set cnt to 0 and save the time buffer
            cnt = 0;
            t1 = TimeBuffer[i];
          } else {
            t2 = TimeBuffer[i];
    ...
    if (DebugCnt > 2){ // checking if there is enough data to be used
      ...
      for (uint32_t i = 1; i < DebugCnt; i++ ){
        ...
        if(cnt2){ 
          ...
          uint32_t pd = Debug_Period(mask); // calculates the period
          return ((diff*100)/pd);
        } 
      }
    } 
  return 0; // average period
}
...
uint32_t Theduty;
void Dump(void){
  uint32_t out = GPIOB->DOUT31_0 & ((1 << 18) |  (1 << 17) | (1 << 16)); // PB18-PB16 outputs
  ...
  if(result == 0){ // 0 means full
    Theperiod = Debug_Period(1<<16); 
    Theduty = Debug_Duty(1<<16);       // calls your Lab3 function
  __asm volatile("bkpt; \n"); // breakpoint here
// observe Theperiod
  Debug_Init();
  }
